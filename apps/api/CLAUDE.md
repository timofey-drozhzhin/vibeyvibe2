...
All CRUD routes are generated by a factory function from a centralized registry of 14 entity configurations. Extension routes handle Spotify import, file upload, and file serving.
...
│   ├── registry.ts       # All 14 entity-context route configs + inline Zod schemas + enricher functions
│   ├── factory/
...
│   └── extensions/
│       ├── anatomy-import.ts # Spotify import preview + confirm endpoints
...
└── validators/
    ├── anatomy.ts        # Legacy Zod schemas (importUrlSchema used by anatomy-import extension)
    ├── bin.ts            # Legacy Zod schemas (not actively used by factory routes)
    ├── my-music.ts       # Legacy Zod schemas (not actively used by factory routes)
    └── suno.ts           # Legacy Zod schemas (not actively used by factory routes)
```

**Key architectural change**: The old per-section route files (`routes/my-music/`, `routes/anatomy/`, `routes/bin/`, `routes/suno/`) and per-section schema files (`schema/my-music.ts`, `schema/anatomy.ts`, `schema/bin.ts`, `schema/suno-studio.ts`) are gone. All CRUD routes are now generated by the factory from registry configurations. Zod schemas used by the factory are defined inline in `routes/registry.ts`. The `validators/` directory contains legacy schemas; only `validators/anatomy.ts` (`importUrlSchema`) is actively consumed by an extension route.
...
Shared entity tables (`songs`, `artists`, `albums`) use a `context` column to differentiate sections. Context values are `"my_music"`, `"anatomy"`, `"bin"`, or `"suno"` depending on the entity.

Complete table inventory:

| Table Name | Type | Description |
|---|---|---|
| `songs` | Entity | Shared songs (context: `my_music` or `anatomy`) |
| `artists` | Entity | Shared artists (context: `my_music` or `anatomy`) |
| `albums` | Entity | Shared albums (context: `my_music` or `anatomy`) |
| `song_profiles` | Entity | Anatomy profiles linked to songs (context: `anatomy`) |
| `song_attributes` | Entity | Anatomy analysis attributes (context: `anatomy`) |
...
All rating fields use `real("rating")` with a **0-1 real scale** (0 = unrated, values between 0 and 1 represent the rating). Zod schemas in the registry currently validate `z.number().min(0).max(5)`. Present on: `songs`, `artists`, `albums`, `song_profiles`, `bin_songs`.
...
Context columns are indexed on shared tables (`songs_context_idx`, `artists_context_idx`, `albums_context_idx`). Foreign key columns on entity tables have indexes (e.g., `song_profiles_song_id_idx`, `bin_songs_bin_source_id_idx`). The `songs` table also indexes `isrc` and `spotify_uid` for duplicate detection during import.
...
interface EntityRouteConfig {
  context: string;           // URL section prefix (e.g., "my-music", "anatomy", "bin", "suno")
  slug: string;              // URL resource name (e.g., "songs", "artists", "song-profiles")
  ...
  contextColumnValue?: string;   // Auto-filter/inject context (e.g., "my_music", "anatomy")
...
The registry (`routes/registry.ts`) defines all 14 entity-context configurations. It also contains all Zod schemas and enricher functions used by those configurations.
...
| `createSunoPromptSchema` | name, lyrics?, prompt?, notes?, song_profile_id? |
...
| 4 | `/anatomy/songs` | `anatomy` | `songs` | Song | list: artists, detail: artists+albums | artists, albums | -- |
| 5 | `/anatomy/artists` | `anatomy` | `artists` | Artist | -- | -- | -- |
| 6 | `/anatomy/albums` | `anatomy` | `albums` | Album | -- | -- | -- |
| 7 | `/anatomy/song-attributes` | `anatomy` | `songAttributes` | Attribute | -- | -- | `category` (eq) |
| 8 | `/anatomy/song-profiles` | `anatomy` | `songProfiles` | Profile | list: songName, detail: songName | -- | `song_id` (eq) |
| 9 | `/bin/sources` | `bin` | `binSources` | Source | -- | -- | -- |
| 10 | `/bin/songs` | `bin` | `binSongs` | Bin Song | list: source, detail: source | -- | `bin_source_id` (eq) |
| 11 | `/suno/prompt-collections` | `suno` | `sunoPromptCollections` | Collection | detail: prompts | prompts | -- |
| 12 | `/suno/prompts` | `suno` | `sunoPrompts` | Prompt | detail: profile | -- | -- |
| 13 | `/suno/song-playlists` | `suno` | `sunoSongPlaylists` | Playlist | -- | -- | -- |
| 14 | `/suno/songs` | `suno` | `sunoSongs` | Suno Song | -- | -- | -- |

### Enricher Functions (defined in registry.ts)

| Function | Used By | Description |
|---|---|---|
| `songListEnricher` | my-music/songs, anatomy/songs | Batch-loads artist names via `artist_songs` pivot for list rows |
| `songDetailEnricher` | my-music/songs, anatomy/songs | Loads related artists and albums for a single song |
| `profileListEnricher` | anatomy/song-profiles | Batch-loads song names for profile list rows |
| `profileDetailEnricher` | anatomy/song-profiles | Loads song name for a single profile |
...
| `sunoPromptDetailEnricher` | suno/prompts | Loads linked song profile for a single prompt |

### Shared Relationship Configs

The `songRelationships` array is reused by both `my-music/songs` and `anatomy/songs`:
...
- `routes.route("/anatomy", anatomyImport)` -- Handles `/anatomy/import` and `/anatomy/import/confirm`
...
- `POST /api/anatomy/import` -- Spotify import preview
- `POST /api/anatomy/import/confirm` -- Spotify import confirm
...
All Zod schemas consumed by factory routes are defined in `routes/registry.ts`, NOT in `validators/`. The `validators/` directory contains legacy schemas from the old per-section route architecture. Only `validators/anatomy.ts` is actively used (its `importUrlSchema` is consumed by `extensions/anatomy-import.ts`).
...
All rating fields in Zod schemas use `z.number().min(0).max(5).nullable().optional()`. The database stores ratings as `real` columns (0-1 scale). Applies to songs, artists, albums, profiles, and bin songs.
...
### Import Routes (extensions/anatomy-import.ts)

- `POST /api/anatomy/import` -- Preview: accepts a Spotify URL and returns extracted tracks for review. Validates URL with `importUrlSchema` from `validators/anatomy.ts`.
- `POST /api/anatomy/import/confirm` -- Confirm: accepts selected tracks and creates records in the `anatomy` context:
  - Creates `songs` records (context: `anatomy`) with duplicate detection by `spotify_uid` and `isrc`
  - Creates `artists` records (context: `anatomy`) with case-insensitive name matching
  - Creates `albums` records (context: `anatomy`) with case-insensitive name matching
