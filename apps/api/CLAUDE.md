...
All CRUD routes are generated by a factory function from a centralized registry of 12 entity configurations. Extension routes handle Spotify import, file upload, and file serving.
...
│   ├── seed.ts           # Database seeding script — sample data (run via pnpm db:seed)
│   └── schema/
...
│   ├── registry.ts       # All 12 entity-context route configs + inline Zod schemas + enricher functions
│   ├── factory/
...
│       ├── profile-generator.ts # AI profile generation (uses static vibes config)
...
| `profiles` | Relational | 1:N from songs, stores AI-generated vibe analysis as JSON |
...
The registry (`routes/registry.ts`) defines all 12 entity-context configurations. It also contains all Zod schemas and enricher functions used by those configurations.
...
Error codes: 404 (song not found), 400 (no active vibes in config), 503 (env not configured), 502 (OpenRouter error), 422 (unparseable response)
