...
└── validators/
    ├── lab.ts            # Legacy Zod schemas (importUrlSchema used by lab-import extension)
...
**Key architectural change**: The old per-section route files (`routes/my-music/`, `routes/lab/`, `routes/bin/`, `routes/suno/`) and per-section schema files (`schema/my-music.ts`, `schema/lab.ts`, `schema/bin.ts`, `schema/suno-studio.ts`) are gone. All CRUD routes are now generated by the factory from registry configurations. Zod schemas used by the factory are defined inline in `routes/registry.ts`. The `validators/` directory contains legacy schemas; only `validators/lab.ts` (`importUrlSchema`) is actively consumed by an extension route.
...
| `profileListEnricher` | lab/song-profiles | Batch-loads song names for profile list rows |
| `profileDetailEnricher` | lab/song-profiles | Loads song name for a single profile |
...
All Zod schemas consumed by factory routes are defined in `routes/registry.ts`, NOT in `validators/`. The `validators/` directory contains legacy schemas from the old per-section route architecture. Only `validators/lab.ts` is actively used (its `importUrlSchema` is consumed by `extensions/lab-import.ts`).
...
- `POST /api/lab/import` -- Preview: accepts a Spotify URL and returns extracted tracks for review. Validates URL with `importUrlSchema` from `validators/lab.ts`.
