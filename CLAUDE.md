...
**Schema**: `{ url: string }` (must be a valid URL, validated by `importUrlSchema` defined inline in `routes/extensions/lab-import.ts`)
...
### Frontend
The Profiles section appears on song show pages as a one-to-many relationship. It shows the last 10 non-archived profiles with columns for date (clickable to view full JSON) and method. Each row has a "Suno Prompt" action button and archive toggle. The "Generate from Vibes" button in the section header creates new profiles. The generate button is configured via a `generateAction` property on `RelationshipDef` in the entity registry. `generateAction` accepts a single `GenerateActionDef` or an array for multiple actions. The `RelationshipSection` component renders action buttons in the section header. Each action can specify `color`, `icon` ("sparkles" or "music"), and optional `successNavigate` (resource name to navigate to on success, using `data.id` from the response).
...
- **`SectionCard`** (defined in `entity-page.tsx`) provides card sections with `Title order={4}` header and optional action button (`size="xs" variant="light"` with `IconPlus size={14}`).
- **`ImageUpload`** click-to-upload with hover overlay.
...
| `SectionCard` | `entity-page.tsx` | Card section with Title order={4} header and optional action button (size="xs" variant="light"). Defined in `entity-page.tsx`. |
| `AudioUpload` | `audio-upload.tsx` | Audio file upload with drag-drop support. Shows player with replace button when audio exists. |
...
All validation schemas are defined inline in `routes/registry.ts` (for factory routes) or in their respective extension route files (e.g., `importUrlSchema` in `routes/extensions/lab-import.ts`). There are no separate validator files.
